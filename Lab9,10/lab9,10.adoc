...
Данный класс нужен для описания интерфейса кнопки.
[source, cpp]
#pragma once
class IButton
{
...
Данный класс определеяет интерфейс, которые будут наследовать моды.
[source, cpp]
#pragma once
class ILed
{
...
Виртуальный класс, поскольку для построения мода в функции должно меняться значение индекса, поэтому константой ее оставлять нельзя.
...
в нем реализуется две функции: функции переключения светодиодов и функция выключения светодиодов. Вторая функция необходима для того, чтобы при переключении режима, новый режим начинался с первого светодиода и программа работала корректно. Для этих двух функций нам необходимы два регистра ODR и BSRR.
[source, cpp]
#pragma once
...
=== Класс leds
Данный класс реализует способ корректноЙ работы мода, для этого создаем масиив, который содержит все пины светодиодов и вызываем функции реализованные в классе Led.
[source, cpp]
#pragma once
#include "iLed.h"
#include "Leed.h"
...
Класс необходимый для реализации работы кнопки. В данном классе используется булевая логика. Для этого мы переопределяем функцию виртуального класса и прописываем условие. Также необходимо предусмотреть защиту от дребезга, чтобы при нажатии кнопка не проскакивала несколько режимов.
...
Класс реализует мод, когда моргают все светодиоды одновременно.
...
Реализует мод елочкой, то есть все свтодиоды включаются и выключаются.
...
Класс реализует мод шахматки, то есть сначала включаются 1 и 3, а потом 2 и 4.
...
Класс реализует механизм переключения модов, с помощью смещения индекса мода на единицу.
...
В нем мы настраиваем внутренний генератор, светодиоды, таймер как мы делали это в предыдущих работах.
